...
try:
    ...
    # Set a lower volume to prevent sudden loud sounds
...
try:
    ...
    for i in range(1, 10):
        try:
            sound_file = f"{i}."
            counting_sounds[i] = pygame.mixer.Sound(sound_file)
            # Set volume for counting sounds (slightly lower than effects)
            counting_sounds[i].set_volume(0.4)
            print(f"Loaded counting sound: {sound_file}")
        except Exception as e:
            print(f"Warning: Could not load counting sound {i}.MP3: {e}")
            counting_sounds[i] = None
    
    # Load number pronunciation sounds (for target numbers in COUNTING mode)
    # Assuming files are named like "number_1.MP3", "number_2.MP3", etc.
    for i in range(1, 10):
        try:
            sound_file = f"number_{i}.MP3"
            number_pronunciation[i] = pygame.mixer.Sound(sound_file)
            number_pronunciation[i].set_volume(0.5)
            print(f"Loaded number pronunciation: {sound_file}")
        except:
            # If specific number pronunciation files don't exist, we'll use counting sounds
            number_pronunciation[i] = counting_sounds[i] if i in counting_sounds else None
...
def play_sound(sound, allow_interrupt=True):
...
def new_counting_question():
    ...
    # Play the target number pronunciation if available
...
def new_addition_question():
    ...
    # Play addition question sound if we have the sounds
    if audio_loaded:
        # You could add addition-specific sounds here if available
        pass
...
# -----------------------------
print(f"Starting {GAME_MODE} game...")
...
# -----------------------------
# MAIN GAME LOOP WITH ENHANCED AUDIO
# -----------------------------
while running:
    dt = clock.tick(30) / 1000.0
                ...
                for apple in apples:
                    if not apple["picked"] and not apple["in_basket"]:
                        ...
                        if dist((apple_center_x, apple_center_y), index_pos) < APPLE_SIZE // 2:
                            ...
                            # PLAY COUNTING SOUND WHEN FIRST PINCHING AN APPLE
    ...
    if not current_is_pinch and pinch_active:
        ...
        if picked_apple:
            ...
            if inside(apple_center_x, apple_center_y, basket):
                ...
                # Play success sound for placing apple in basket (optional)
                # You could add a "plop" or "drop" sound effect here
    ...
    # Draw game mode indicator - ONLY for ADDITION mode
    if GAME_MODE == "ADDITION":
        mode_bg = pygame.Surface((300, 60), pygame.SRCALPHA)
        mode_bg.fill((0, 0, 0, 150))
        screen.blit(mode_bg, (WIDTH - 320, 30))
        
        mode_text = font.render(f"Mode: {GAME_MODE}", True, (100, 255, 150))
        screen.blit(mode_text, (WIDTH - 310, 40))
    ...
    else:  # ADDITION mode (unchanged)
        # Draw problem as before
        problem_bg = pygame.Surface((600, 100), pygame.SRCALPHA)
        problem_bg.fill((0, 0, 0, 180))
        screen.blit(problem_bg, ((WIDTH - 600)//2, 50))
        
        problem_display = large_font.render(problem_text, True, (255, 200, 150))
        screen.blit(problem_display, ((WIDTH - problem_display.get_width())//2, 70))
        
        # Draw target and current count
        count_bg = pygame.Surface((400, 80), pygame.SRCALPHA)
        count_bg.fill((0, 0, 0, 150))
        screen.blit(count_bg, (40, 30))
        
        count_text = font.render(f"Target: {target_count} | Current: {dropped}", True, (255, 255, 255))
        screen.blit(count_text, (50, 50))
        
        # Draw small apples (circles) for ADDITION mode
        apple_y = 200
        start_x = 50
        for i in range(target_count):
            apple_x = start_x + i * 90
            if i < dropped:
                pygame.draw.circle(screen, (255, 50, 50), (apple_x, apple_y), 30)
                pygame.draw.circle(screen, (200, 30, 30), (apple_x, apple_y), 30, 2)
            else:
                pygame.draw.circle(screen, (255, 255, 255), (apple_x, apple_y), 30, 2)
    ...
    # Draw current counting status (visual feedback for audio)
    if audio_loaded and dropped > 0 and dropped <= 9:
        count_status = small_font.render(f"Counted: {dropped}", True, (100, 255, 100))
        screen.blit(count_status, (WIDTH - 200, 200))
...
print(f"  Accuracy: {(correct_problems/total_problems*100):.1f}%" if total_problems > 0 else "Accuracy: N/A")
print(f"  Session Duration: {TOTAL_SCREEN_TIME:.1f} seconds")
print(f"  Multimodal Features: Gesture-based manipulation with synchronized auditory counting")
