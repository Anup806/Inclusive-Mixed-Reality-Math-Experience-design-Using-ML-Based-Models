...
def new_counting_question():
    ...
    return target, f"Count to {target}"
...
while game_selection_active and running:
    ...
    for button in mode_buttons:
        ...
        desc_text = small_font.render(
            "Drag apples to match the number" if button["mode"] == "COUNTING" else "Solve addition problems",
            True, (220, 220, 220)
        )
        desc_rect = desc_text.get_rect(center=(button["rect"].centerx, button["rect"].bottom + 30))
    ...
    # Draw instructions
    instr_bg = pygame.Surface((WIDTH, 60), pygame.SRCALPHA)
    instr_bg.fill((0, 0, 0, 150))
    screen.blit(instr_bg, (0, HEIGHT - 100))
    
    if finger_pos:
        instr_text = small_font.render(f"Move hand to hover over a mode, then PINCH to select | Hand at: {finger_pos}", True, (255, 255, 200))
    else:
        instr_text = small_font.render("Show your hand to the camera to select a game mode", True, (255, 200, 200))
    
    screen.blit(instr_text, ((WIDTH - instr_text.get_width())//2, HEIGHT - 80))
...
while running:
    ...
    # Draw problem with better styling - adjust width based on mode
    if GAME_MODE == "COUNTING":
        problem_bg_width = 800
    else:
        problem_bg_width = 600
        
    problem_bg = pygame.Surface((problem_bg_width, 100), pygame.SRCALPHA)
    problem_bg.fill((0, 0, 0, 180))
    screen.blit(problem_bg, ((WIDTH - problem_bg_width)//2, 50))
    
    if GAME_MODE == "COUNTING":
        problem_display = large_font.render(problem_text, True, (255, 255, 200))
    else:  # ADDITION
    ...
    # Draw target and current count - ONLY for ADDITION mode
    if GAME_MODE == "ADDITION":
    ...
    # Draw small apples visualization
    apple_y = 200
    # Center the apples based on mode
    if GAME_MODE == "COUNTING":
    ...
    for i in range(target_count):
        ...
        if i < dropped:
            ...
            # Apple not yet in basket (outline)
