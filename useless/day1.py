...
submit = {"x": WIDTH - 300, "y": HEIGHT - 160, "w": 220, "h": 90}
...
def reset_game():
    ...
    global num1, num2, target_count, dropped, apples, message, message_time
    num1, num2, target_count = new_question()
    dropped = 0
...
def compute_pinch_state(hand_landmarks, img_w, img_h, previous_state=False):
    """
    Stable pinch detection with hysteresis
    """
    thumb = hand_landmarks.landmark[4]
    index = hand_landmarks.landmark[8]

    tx, ty = thumb.x * img_w, thumb.y * img_h
    ix, iy = index.x * img_h, index.y * img_h
...
if current_is_pinch and not pinch_active:
    if now - last_pinch_time > PINCH_DELAY:
        ...
        if index_pos:
            ...
            for apple in apples:
                if apple["picked"] or apple["in_basket"]:
                    continue

                cx = apple["x"] + APPLE_SIZE // 2
                cy = apple["y"] + APPLE_SIZE // 2
                d = dist((cx, cy), index_pos)

                if d < APPLE_SIZE // 2 and d < closest_dist:
                    closest_dist = d
                    picked_apple = apple

        # Check submit AFTER apple
        if picked_apple is None and hand_over_submit:
            submit_pinched = True
    ...
    # if not current_is_pinch and pinch_active and index_pos:
    #     # Check if fingers are far apart
    #     thumb_pos, idx_pos = get_finger_positions(handLms, WIDTH, HEIGHT) if result and result.multi_hand_landmarks else (None, None)
    #     if thumb_pos and idx_pos:
    #         distance = dist(thumb_pos, idx_pos)
    ...
    if pinch_active and not current_is_pinch:
        pinch_active = False
    ...
    #pygame.draw.rect(screen, (255, 255, 255), (40, 30, 400, 80))
    #pygame.draw.rect(screen, (0, 0, 0), (40, 30, 400, 80), 2)
    
    stats_text = font.render(f"{target_count} Apples", True, (0, 0, 0))
